export default function BenefitsPage() {
  return (
    <div>
      <div className="items-center bg-white flex w-full flex-col px-20 py-12 max-md:max-w-full max-md:px-5">
        <div className="text-black text-center text-6xl font-bold leading-[67px] max-w-screen-md mt-16 max-md:max-w-full max-md:text-4xl max-md:leading-[54px] max-md:mt-10">
          Experience Warmth
        </div>
        <div className="text-black text-center text-lg leading-7 max-w-screen-md mt-6 mb-10 max-md:max-w-full">
          Discover the benefits of our underfloor heating system and enjoy a
          comfortable home all year round.
        </div>
      </div>
      <div className="items-stretch bg-white flex w-full flex-col justify-center px-16 py-12 max-md:max-w-full max-md:px-5">
        <div className="mt-16 mb-10 max-md:max-w-full max-md:mr-1 max-md:mt-10">
          <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
            <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
              <div className="items-stretch flex flex-col my-auto max-md:max-w-full max-md:mt-10">
                <div className="text-black text-4xl font-bold leading-10 max-md:max-w-full">
                  The Only Heating Source You Need for a Comfortable Indoor
                  Climate
                </div>
                <div className="text-black text-lg leading-7 mt-6 max-md:max-w-full">
                  Our underfloor heating system serves as the primary heat
                  source, providing the most comfortable and uniform level of
                  warmth. With our system, you won't need any other heating
                  systems.
                </div>
                <div className="mt-8 py-2 max-md:max-w-full">
                  <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
                    <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                      <div className="flex grow flex-col items-start max-md:mt-8">
                        <picture>
                          <img
                            loading="lazy"
                            src="https://cdn.builder.io/api/v1/image/assets/TEMP/948451c9cd8add0c7dc0bef71b0473d97863fd278817d8c452b8ceeb18b02e5b?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                            className="aspect-square object-contain object-center w-12 overflow-hidden max-w-full"
                          />
                        </picture>
                        <div className="self-stretch text-black text-xl font-bold leading-7 whitespace-nowrap mt-4">
                          Affordable
                        </div>
                        <div className="self-stretch text-black text-base leading-6 mt-4">
                          No sub-floor work needed, making our underfloor
                          heating system a cost-effective choice.
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                      <div className="flex flex-col items-start max-md:mt-8">
                        <picture>
                          <img
                            loading="lazy"
                            src="https://cdn.builder.io/api/v1/image/assets/TEMP/948451c9cd8add0c7dc0bef71b0473d97863fd278817d8c452b8ceeb18b02e5b?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                            className="aspect-square object-contain object-center w-12 overflow-hidden max-w-full"
                          />
                        </picture>
                        <div className="self-stretch text-black text-xl font-bold leading-7 whitespace-nowrap mt-4">
                          Quick
                        </div>
                        <div className="self-stretch text-black text-base leading-6 mt-4">
                          Installations take only one working day, saving you
                          time and hassle.
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
              <picture>
                <img
                  loading="lazy"
                  srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-[0.96] object-contain object-center w-full overflow-hidden grow max-md:max-w-full max-md:mt-10"
                />
              </picture>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col justify-center px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="mt-16 mb-10 max-md:max-w-full max-md:mr-1 max-md:mt-10">
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                <div className="items-start flex flex-col my-auto max-md:max-w-full max-md:mt-10">
                  <div className="text-black text-center text-base font-semibold leading-6 self-stretch max-md:max-w-full">
                    Efficient
                  </div>
                  <div className="self-stretch text-black text-5xl font-bold leading-[58px] mt-4 max-md:max-w-full max-md:text-4xl max-md:leading-[54px]">
                    Cost-Effective Solution for Underfloor Heating Systems
                  </div>
                  <div className="self-stretch text-black text-lg leading-7 mt-6 max-md:max-w-full">
                    Our underfloor heating system offers a cost-effective
                    solution for homeowners. With no sub-floor work required, it
                    is an affordable option compared to traditional methods.
                  </div>
                  <div className="self-stretch mt-8 py-2 max-md:max-w-full">
                    <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
                      <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                        <div className="items-stretch flex grow flex-col max-md:mt-8">
                          <div className="text-black text-5xl font-bold leading-[57.6px] whitespace-nowrap max-md:text-4xl">
                            50%
                          </div>
                          <div className="text-black text-base leading-6 whitespace-nowrap mt-2">
                            No Sub-Floor Work Needed
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                        <div className="items-stretch flex grow flex-col max-md:mt-8">
                          <div className="text-black text-5xl font-bold leading-[57.6px] whitespace-nowrap max-md:text-4xl">
                            50%
                          </div>
                          <div className="text-black text-base leading-6 whitespace-nowrap mt-2">
                            Affordable and Efficient Heating Solution
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                <picture>
                  <img
                    loading="lazy"
                    srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                    className="aspect-[0.96] object-contain object-center w-full overflow-hidden grow max-md:max-w-full max-md:mt-10"
                  />
                </picture>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="mt-16 max-md:max-w-full max-md:mt-10">
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                <div className="items-stretch flex flex-col max-md:max-w-full max-md:mt-10">
                  <div className="text-black text-base font-semibold leading-6 whitespace-nowrap max-md:max-w-full">
                    Efficient
                  </div>
                  <div className="text-black text-5xl font-bold leading-[58px] mt-4 max-md:max-w-full max-md:text-4xl max-md:leading-[54px]">
                    Quick and Easy Installation Process for Underfloor Heating
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                <div className="items-stretch flex grow flex-col max-md:max-w-full max-md:mt-10">
                  <div className="text-black text-lg leading-7 max-md:max-w-full">
                    Our underfloor heating system offers a quick and easy
                    installation process, allowing you to have a fully
                    functional system in just one working day. With our patented
                    floorgrinder, we create channels in your existing floor
                    construction, where the underfloor heating pipes are laid.
                    This method is dust-free and requires no sub-floor work,
                    making it a hassle-free solution for your heating needs.
                  </div>
                </div>
              </div>
            </div>
          </div>
          <picture>
            <img
              loading="lazy"
              srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/d7fa4f3a68eaeba4ad6b5be86a3cf4a38e7b197b88567b2671cdf56d7de036e4?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/d7fa4f3a68eaeba4ad6b5be86a3cf4a38e7b197b88567b2671cdf56d7de036e4?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/d7fa4f3a68eaeba4ad6b5be86a3cf4a38e7b197b88567b2671cdf56d7de036e4?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/d7fa4f3a68eaeba4ad6b5be86a3cf4a38e7b197b88567b2671cdf56d7de036e4?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/d7fa4f3a68eaeba4ad6b5be86a3cf4a38e7b197b88567b2671cdf56d7de036e4?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/d7fa4f3a68eaeba4ad6b5be86a3cf4a38e7b197b88567b2671cdf56d7de036e4?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/d7fa4f3a68eaeba4ad6b5be86a3cf4a38e7b197b88567b2671cdf56d7de036e4?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/d7fa4f3a68eaeba4ad6b5be86a3cf4a38e7b197b88567b2671cdf56d7de036e4?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
              className="aspect-[1.78] object-contain object-center w-full overflow-hidden mt-20 mb-10 max-md:max-w-full max-md:mt-10"
            />
          </picture>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col justify-center px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="mt-16 mb-10 max-md:max-w-full max-md:mr-1 max-md:mt-10">
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                <div className="items-stretch flex flex-col my-auto max-md:max-w-full max-md:mt-10">
                  <div className="text-black text-4xl font-bold leading-10 max-md:max-w-full">
                    The Zero Build-Up Advantage: Embedded 'Into' the Screed
                    Floor
                  </div>
                  <div className="text-black text-lg leading-7 mt-6 max-md:max-w-full">
                    Our underfloor heating system offers the unique benefit of
                    being embedded 'into' the screed floor without any increase
                    in floor level. This means you can enjoy the comfort of
                    underfloor heating without compromising on space or
                    aesthetics.
                  </div>
                  <div className="mt-8 py-2 max-md:max-w-full">
                    <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
                      <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                        <div className="items-stretch flex grow flex-col max-md:mt-8">
                          <div className="text-black text-xl font-bold leading-7 whitespace-nowrap">
                            Subheading one
                          </div>
                          <div className="text-black text-base leading-6 mt-4">
                            Effortless Installation: No Need to Raise or Remove
                            Existing Screed
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                        <div className="items-stretch flex grow flex-col max-md:mt-8">
                          <div className="text-black text-xl font-bold leading-7 whitespace-nowrap">
                            Subheading two
                          </div>
                          <div className="text-black text-base leading-6 mt-4">
                            Experience Immediate Warmth: Feel the Comfort Within
                            Minutes
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                <picture>
                  <img
                    loading="lazy"
                    srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                    className="aspect-[0.96] object-contain object-center w-full overflow-hidden grow max-md:max-w-full max-md:mt-10"
                  />
                </picture>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col justify-center px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="mt-16 mb-10 max-md:max-w-full max-md:mr-1 max-md:mt-10">
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                <div className="items-start flex flex-col my-auto max-md:max-w-full max-md:mt-10">
                  <div className="text-black text-center text-base font-semibold leading-6 self-stretch max-md:max-w-full">
                    Tailored
                  </div>
                  <div className="self-stretch text-black text-5xl font-bold leading-[58px] mt-4 max-md:max-w-full max-md:text-4xl max-md:leading-[54px]">
                    Efficient and Customized Cutting Process for Optimal Results
                  </div>
                  <div className="self-stretch text-black text-lg leading-7 mt-6 max-md:max-w-full">
                    Our cutting process is tailored specifically to each site,
                    ensuring optimal efficiency and results. With our bespoke
                    approach, we are able to provide a customized solution that
                    meets the unique needs of your project. By utilizing our
                    patented floorgrinder, we are able to create precise
                    channels in existing floors without producing any dust. This
                    allows for a quick and hassle-free installation process,
                    with installations typically completed in just one working
                    day. Experience the benefits of our efficient and
                    eco-friendly cutting process today.
                  </div>
                  <div className="items-stretch flex justify-between gap-5 mt-6 pt-4 self-start">
                    <div className="justify-center items-stretch flex gap-2 mt-3 self-start">
                      <div className="text-black text-base leading-6 grow whitespace-nowrap">
                        Contact Us
                      </div>
                      <picture>
                        <img
                          loading="lazy"
                          src="https://cdn.builder.io/api/v1/image/assets/TEMP/9742a420396e5caa7e429fdf2ccb9079b0158ce1d368acae84bfdb7e55682093?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                          className="aspect-square object-contain object-center w-6 overflow-hidden shrink-0 max-w-full"
                        />
                      </picture>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                <picture>
                  <img
                    loading="lazy"
                    srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                    className="aspect-[0.96] object-contain object-center w-full overflow-hidden grow max-md:max-w-full max-md:mt-10"
                  />
                </picture>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col justify-center px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="mt-16 mb-10 max-md:max-w-full max-md:mr-1 max-md:mt-10">
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                <div className="items-start flex flex-col my-auto max-md:max-w-full max-md:mt-10">
                  <div className="text-black text-center text-base font-semibold leading-6 self-stretch max-md:max-w-full">
                    Efficient
                  </div>
                  <div className="self-stretch text-black text-5xl font-bold leading-[58px] mt-4 max-md:max-w-full max-md:text-4xl max-md:leading-[54px]">
                    Experience the Immediate Warmth and Comfort
                  </div>
                  <div className="self-stretch text-black text-lg leading-7 mt-6 max-md:max-w-full">
                    Our underfloor heating system is incredibly responsive,
                    providing warmth within minutes of being turned on. Say
                    goodbye to cold floors and experience the immediate comfort
                    of our low temperature heating system.
                  </div>
                  <div className="self-stretch mt-8 py-2 max-md:max-w-full">
                    <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
                      <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                        <div className="flex grow flex-col items-start max-md:mt-8">
                          <picture>
                            <img
                              loading="lazy"
                              src="https://cdn.builder.io/api/v1/image/assets/TEMP/948451c9cd8add0c7dc0bef71b0473d97863fd278817d8c452b8ceeb18b02e5b?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                              className="aspect-square object-contain object-center w-12 overflow-hidden max-w-full"
                            />
                          </picture>
                          <div className="self-stretch text-black text-xl font-bold leading-7 whitespace-nowrap mt-4">
                            Immediate Comfort
                          </div>
                          <div className="self-stretch text-black text-base leading-6 mt-4">
                            Feel the warmth within minutes and enjoy the
                            immediate comfort of our underfloor heating system.
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                        <div className="flex grow flex-col items-start max-md:mt-8">
                          <picture>
                            <img
                              loading="lazy"
                              src="https://cdn.builder.io/api/v1/image/assets/TEMP/948451c9cd8add0c7dc0bef71b0473d97863fd278817d8c452b8ceeb18b02e5b?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                              className="aspect-square object-contain object-center w-12 overflow-hidden max-w-full"
                            />
                          </picture>
                          <div className="self-stretch text-black text-xl font-bold leading-7 whitespace-nowrap mt-4">
                            Energy Efficient
                          </div>
                          <div className="self-stretch text-black text-base leading-6 mt-4">
                            Our low temperature heating system uses very low
                            water temperatures, resulting in energy savings.
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                <img
                  loading="lazy"
                  srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-[0.96] object-contain object-center w-full overflow-hidden grow max-md:max-w-full max-md:mt-10"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col justify-center px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="mt-16 mb-10 max-md:max-w-full max-md:mr-1 max-md:mt-10">
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                <div className="items-stretch flex flex-col my-auto max-md:max-w-full max-md:mt-10">
                  <div className="text-black text-4xl font-bold leading-10 max-md:max-w-full">
                    Energy Efficiency: Operating at Low Water Temperatures for
                    Lower Energy Bills
                  </div>
                  <div className="text-black text-lg leading-7 mt-6 max-md:max-w-full">
                    Our underfloor heating system is designed to operate at very
                    low water temperatures, making it highly energy efficient.
                    By utilizing the low temperature heating (LTH) technology,
                    our system can easily integrate with existing heating
                    systems, resulting in lower energy usage and reduced energy
                    bills.
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                <picture>
                  <img
                    loading="lazy"
                    srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                    className="aspect-[0.96] object-contain object-center w-full overflow-hidden grow max-md:max-w-full max-md:mt-10"
                  />
                </picture>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col justify-center px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="mt-16 mb-10 max-md:max-w-full max-md:mr-1 max-md:mt-10">
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                <div className="items-start flex flex-col my-auto max-md:max-w-full max-md:mt-10">
                  <div className="text-black text-center text-base font-semibold leading-6 self-stretch max-md:max-w-full">
                    Efficiency
                  </div>
                  <div className="self-stretch text-black text-5xl font-bold leading-[58px] mt-4 max-md:max-w-full max-md:text-4xl max-md:leading-[54px]">
                    Reduced Gas Bills and Sustainability
                  </div>
                  <div className="self-stretch text-black text-lg leading-7 mt-6 max-md:max-w-full">
                    Discover the environmental benefits of our underfloor
                    heating system. Our low temperature heating method can lead
                    to reduced gas bills and support a more sustainable
                    lifestyle.
                  </div>
                  <div className="self-stretch mt-8 py-2 max-md:max-w-full">
                    <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
                      <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                        <div className="flex flex-col items-start max-md:mt-8">
                          <img
                            loading="lazy"
                            src="https://cdn.builder.io/api/v1/image/assets/TEMP/948451c9cd8add0c7dc0bef71b0473d97863fd278817d8c452b8ceeb18b02e5b?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                            className="aspect-square object-contain object-center w-12 overflow-hidden max-w-full"
                          />
                          <div className="self-stretch text-black text-xl font-bold leading-7 whitespace-nowrap mt-4">
                            Lower Bills
                          </div>
                          <div className="self-stretch text-black text-base leading-6 mt-4">
                            Save money on your gas bill while reducing your
                            carbon footprint.
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                        <div className="flex grow flex-col items-start max-md:mt-8">
                          <img
                            loading="lazy"
                            src="https://cdn.builder.io/api/v1/image/assets/TEMP/948451c9cd8add0c7dc0bef71b0473d97863fd278817d8c452b8ceeb18b02e5b?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                            className="aspect-square object-contain object-center w-12 overflow-hidden max-w-full"
                          />
                          <div className="self-stretch text-black text-xl font-bold leading-7 whitespace-nowrap mt-4">
                            Sustainable Living
                          </div>
                          <div className="self-stretch text-black text-base leading-6 mt-4">
                            Support a more sustainable lifestyle with our
                            energy-efficient underfloor heating system.
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                <img
                  loading="lazy"
                  srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/695e35fe9375ed813e9267c48b6e6e0ed2c5303c0a4eb46842040dcb8370ac1e?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-[0.96] object-contain object-center w-full overflow-hidden grow max-md:max-w-full max-md:mt-10"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="justify-between items-stretch flex gap-5 mt-16 max-md:max-w-full max-md:flex-wrap max-md:justify-center max-md:mt-10">
            <div className="justify-center items-center border self-center flex aspect-square flex-col w-12 h-12 my-auto px-3 rounded-[50px] border-solid border-black">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/be0cb1c1b927ed61f8c3fe13755809ee3391e41f04bfe4c53c4bf9f26a634c86?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                className="aspect-square object-contain object-center w-full overflow-hidden"
              />
            </div>
            <div className="items-stretch flex grow basis-[0%] flex-col max-md:max-w-full">
              <div className="items-stretch self-center flex w-[116px] max-w-full gap-1 max-md:justify-center">
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/7e4df2cd1a7939a31855fa8e82b1b9421fc9a6af4643291313cbb9984d060bb0?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-[1.05] object-contain object-center w-full fill-black overflow-hidden shrink-0 flex-1"
                />
              </div>
              <div className="text-black text-center text-2xl font-bold leading-9 mt-8 max-md:max-w-full">
                Our underfloor heating system has completely transformed the way
                we experience warmth in our home. It provides the most
                comfortable and uniform level of warmth, making it the only and
                primary heat source we need.
              </div>
              <div className="items-center self-center flex justify-between gap-5 mt-8 py-1 max-md:max-w-full max-md:flex-wrap max-md:justify-center">
                <picture>
                  <img
                    loading="lazy"
                    srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/0df769b8fb553ad7114a5389beeaf1cd3b26f19398cdc0cefc0d68fcffb12a9c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/0df769b8fb553ad7114a5389beeaf1cd3b26f19398cdc0cefc0d68fcffb12a9c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/0df769b8fb553ad7114a5389beeaf1cd3b26f19398cdc0cefc0d68fcffb12a9c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/0df769b8fb553ad7114a5389beeaf1cd3b26f19398cdc0cefc0d68fcffb12a9c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/0df769b8fb553ad7114a5389beeaf1cd3b26f19398cdc0cefc0d68fcffb12a9c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/0df769b8fb553ad7114a5389beeaf1cd3b26f19398cdc0cefc0d68fcffb12a9c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/0df769b8fb553ad7114a5389beeaf1cd3b26f19398cdc0cefc0d68fcffb12a9c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/0df769b8fb553ad7114a5389beeaf1cd3b26f19398cdc0cefc0d68fcffb12a9c?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                    className="aspect-square object-contain object-center w-14 overflow-hidden self-stretch shrink-0 max-w-full rounded-[50%]"
                  />
                </picture>
                <div className="items-stretch flex grow basis-[0%] flex-col my-auto">
                  <div className="text-black text-base font-semibold leading-6">
                    John Smith
                  </div>
                  <div className="text-black text-base leading-6 whitespace-nowrap">
                    Homeowner, ABC Company
                  </div>
                </div>
              </div>
            </div>
            <div className="justify-center items-center border self-center flex aspect-square flex-col w-12 h-12 my-auto px-3 rounded-[50px] border-solid border-black">
              <picture>
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/824733054ac46de05730ed2043bb671e454190e751dfe87239c00595f0af935c?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                  className="aspect-square object-contain object-center w-full overflow-hidden"
                />
              </picture>
            </div>
          </div>
          <div className="items-stretch self-center flex w-24 max-w-full gap-2.5 mt-12 mb-10 p-2.5 max-md:justify-center max-md:mt-10">
            <div className="flex shrink-0 h-2 flex-col flex-1 rounded-[50%]" />
            <div className="flex shrink-0 h-2 flex-col flex-1 rounded-[50%]" />
            <div className="flex shrink-0 h-2 flex-col flex-1 rounded-[50%]" />
            <div className="flex shrink-0 h-2 flex-col flex-1 rounded-[50%]" />
            <div className="flex shrink-0 h-2 flex-col flex-1 rounded-[50%]" />
          </div>
        </div>
      </div>
      <div>
        <div className="items-stretch bg-white flex w-full flex-col justify-center px-16 py-12 max-md:max-w-full max-md:px-5">
          <div className="mt-16 mb-10 max-md:max-w-full max-md:mr-1 max-md:mt-10">
            <div className="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
              <div className="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0">
                <div className="flex flex-col my-auto items-start max-md:max-w-full max-md:mt-10">
                  <div className="self-stretch text-black text-5xl font-bold leading-[58px] max-md:max-w-full max-md:text-4xl max-md:leading-[54px]">
                    Discover the Benefits of Underfloor Heating
                  </div>
                  <div className="self-stretch text-black text-lg leading-7 mt-6 max-md:max-w-full">
                    Experience the comfort and energy efficiency of our
                    underfloor heating system.
                  </div>
                  <div className="items-stretch flex gap-4 mt-6 pt-4">
                    <div className="text-white text-base leading-6 whitespace-nowrap justify-center items-stretch border bg-black grow px-6 py-3 border-solid border-black max-md:px-5">
                      Contact
                    </div>
                    <div className="text-black text-base leading-6 whitespace-nowrap justify-center items-stretch border grow px-6 py-3 border-solid border-black max-md:px-5">
                      Schedule
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-stretch w-6/12 ml-5 max-md:w-full max-md:ml-0">
                <picture>
                  <img
                    loading="lazy"
                    srcSet="https://cdn.builder.io/api/v1/image/assets/TEMP/4112c5730568f09d51d090cdc256de37712f3b8442a5930047844f5a2f1cbe3c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=100 100w, https://cdn.builder.io/api/v1/image/assets/TEMP/4112c5730568f09d51d090cdc256de37712f3b8442a5930047844f5a2f1cbe3c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=200 200w, https://cdn.builder.io/api/v1/image/assets/TEMP/4112c5730568f09d51d090cdc256de37712f3b8442a5930047844f5a2f1cbe3c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=400 400w, https://cdn.builder.io/api/v1/image/assets/TEMP/4112c5730568f09d51d090cdc256de37712f3b8442a5930047844f5a2f1cbe3c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=800 800w, https://cdn.builder.io/api/v1/image/assets/TEMP/4112c5730568f09d51d090cdc256de37712f3b8442a5930047844f5a2f1cbe3c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets/TEMP/4112c5730568f09d51d090cdc256de37712f3b8442a5930047844f5a2f1cbe3c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets/TEMP/4112c5730568f09d51d090cdc256de37712f3b8442a5930047844f5a2f1cbe3c?apiKey=052aa92796d54d509e3efd8cbd8b7989&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets/TEMP/4112c5730568f09d51d090cdc256de37712f3b8442a5930047844f5a2f1cbe3c?apiKey=052aa92796d54d509e3efd8cbd8b7989&"
                    className="aspect-[1.54] object-contain object-center w-full overflow-hidden grow max-md:max-w-full max-md:mt-10"
                  />
                </picture>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
